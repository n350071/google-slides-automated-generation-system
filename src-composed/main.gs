...
const slideData = [
  ...
  // Background knowledge - cards format
  {
    ...
    columns: 2,
    items: [
      { title: 'Google Apps Script?GAS?', desc: 'Google?????JavaScript?????[[????????]]' },
      { title: 'AI?????', desc: 'Gemini?**????**????????' },
      { title: '??IT??', desc: '??AI????????[[????????]]' },
      { title: '?????????????', desc: '?????**???????????**' }
    ],
    notes: '????????????????????????????????????????????????????????'
  ...
  // Key points - cards format
  {
    ...
    items: [
      { title: '??????×?????????????', desc: 'AI?????????????**?????????**???' },
      { title: '???????', desc: '???????GAS?????????AI??[[????JSON?]]????' },
      { title: '????????', desc: 'Google Workspace??????????**??**' },
      { title: '8???????????', desc: 'content?compare?process?timeline??[[?????????]]' },
      { title: '??????', desc: 'AI?????**?????????**?????' }
    ],
    notes: '????5??????????????????????????????????????'
  },
  ...
  // Deep dive topics - cards
  {
    ...
    columns: 3,
    items: [
      { title: 'Google Apps Script', desc: '**????????**?Slides API?????' },
      { title: '?????????????', desc: '???[[????]]??????' },
      { title: 'AI????', desc: '??????????**??????**' },
      { title: '??IT?????', desc: '?????????????[[???]]' },
      { title: 'JSON Schema??', desc: '**?????????**??????????????' }
    ],
    notes: '??????????????????????????????????????????'
  },
...
function createCompareSlide(slide, data, layout, pageNum) {
  ...
  drawCompareBox(slide, leftBox, data.leftTitle || '???A', data.leftItems || []);
  drawCompareBox(slide, rightBox, data.rightTitle || '???B', data.rightItems || []);
...
function createTableSlide(slide, data, layout, pageNum) {
  ...
  try {
    ...
    (headers.length ? headers : ['??','?1','?2']).forEach((h, c) => drawCell(0, c, h, true));
    for (let r = 0; r < rows.length; r++) {
      const row = rows[r] || [];
